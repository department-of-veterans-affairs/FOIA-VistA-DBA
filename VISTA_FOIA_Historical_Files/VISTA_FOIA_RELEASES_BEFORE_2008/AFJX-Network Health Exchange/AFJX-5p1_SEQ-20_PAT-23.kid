...
AFJX*5.1*23
"BLD",546,0)
AFJX*5.1*23^NETWORK HEALTH EXCHANGE^0^3001010^y
"BLD",546,1,0)
^^96^96^3001010^
"BLD",546,1,1,0)
...
AFJXTRF^^0^B10289816
"BLD",546,"KRN",9.8,"NM","B","AFJXALRT",1)

"BLD",546,"KRN",9.8,"NM","B","AFJXPATS",2)

"BLD",546,"KRN",9.8,"NM","B","AFJXREW",3)

"BLD",546,"KRN",9.8,"NM","B","AFJXTRF",4)
...
"BLD",546,"KRN","B",771,771)

"BLD",546,"KRN","B",869.2,869.2)

"BLD",546,"KRN","B",870,870)
...
"RTN","AFJXALRT",2,0)
 ;;5.1;Network Health Exchange;**6,15,17,23**;Jan 23, 1996
"RTN","AFJXALRT",3,0)
...
"RTN","AFJXALRT",6,0)
 S FLAG=0,U="^",CT=0,(I,SEN)=0,DFN="",DIC="^AFJ(537000,",DIC(0)="L",(MMS,X)=XMZ,REC=$G(^XMB(3.9,+XMZ,2,1,0)),SSN=$P(REC,U,1),XMFROM=$P(REC,U,3)_"@"_$P(REC,U,5),WHO=$P(REC,U,2),BCTO=$P(REC,U,5),KIND=$P(REC,U,6)
"RTN","AFJXALRT",7,0)
 K ^TMP("AFJX",$J,SSN)
"RTN","AFJXALRT",8,0)
 S ^TMP("AFJX",$J,SSN,1,0)=REC,RQTM=$P(REC,U,4),CT=3
"RTN","AFJXALRT",9,0)
 G:$G(SSN)="" NOSS
"RTN","AFJXALRT",10,0)
 D NHX
...
"RTN","AFJXALRT",19,0)
 S RETURN="NETWORK,HEALTH EXCHANGE@"_BCTO,XMY(RETURN)="",XMTEXT="^TMP(""AFJX"",$J,SSN,"
"RTN","AFJXALRT",20,0)
 D ^XMD S MESID=XMZ D ALERT
"RTN","AFJXALRT",21,0)
HTOG K XMSUB,XMTEXT,XMY,^TMP("AFJX",$J,SSN)
"RTN","AFJXALRT",22,0)
 Q
"RTN","AFJXALRT",23,0)
GATH ;GATHER DATA %%%%%%
"RTN","AFJXALRT",24,0)
 I $G(DFN)="" S DFN="NO DFN",^TMP("AFJX",$J,SSN,1,0)="Social Security Number: "_$G(SSN),^TMP("AFJX",$J,SSN,2,0)="*****  NO PATIENT DATA FOUND FOR THIS REQUEST  *****" D SENDM G UNSEC
"RTN","AFJXALRT",25,0)
NOSS ;IF NO SSN
"RTN","AFJXALRT",26,0)
 I $G(SSN)="" S ^TMP("AFJX",$J,SSN,1,0)="NO Social Security Number ",^TMP("AFJX",$J,SSN,2,0)="**** PATIENT DATA REQUEST UNSUCESSFUL TRY AGAIN ****",^TMP("AFJX",$J,SSN,3,0)="*****  SOCIAL SECURITY NUMBER NOT DEFINED  *****" D SENDM G UNSEC
"RTN","AFJXALRT",27,0)
...
"RTN","AFJXALRT",41,0)
 S XMSUB="NETWORK HEALTH EXCHANGE REQUESTED FOR SENSITIVE PATIENT",XMY(NHXU)="",XMTEXT="^TMP(""AFJXSEN"",$J,SSN," D ^XMD K XMSUB,XMTEXT,XMY,^TMP("AFJXSEN",$J) Q
"RTN","AFJXALRT",42,0)
WTTB D DUPLI
"RTN","AFJXALRT",43,0)
 Q:$G(FLAG)=1
...
0^2^B4996183
"RTN","AFJXPATS",1,0)
AFJXPATS ;FJ/CWS;NETWORK HEALTH CROSS REF PATIENT;11/8/95 ;1/17/96  13:19
"RTN","AFJXPATS",2,0)
 ;;5.1;Network Health Exchange;**23**;Jan 23, 1996
"RTN","AFJXPATS",3,0)
START ;receive data from patient file 0 node
"RTN","AFJXPATS",4,0)
 S U="^",CT=5
"RTN","AFJXPATS",5,0)
 S ZTSAVE("*")="",ZTIO="",ZTRTN="FIRST^AFJXPATS",ZTDESC="NETWORK PATIENT LIST" D ^%ZTLOAD K ZTRTN,ZTIO,ZTDESC Q
"RTN","AFJXPATS",6,0)
FIRST D BEG,SEND
"RTN","AFJXPATS",7,0)
 Q
"RTN","AFJXPATS",8,0)
BEG S DFN="" F  S DFN=$O(^DPT(DFN)) Q:DFN=""  D WRITE
"RTN","AFJXPATS",9,0)
 Q
"RTN","AFJXPATS",10,0)
TOP ;
"RTN","AFJXPATS",11,0)
 D NOW^%DTC S ASOF=%
"RTN","AFJXPATS",12,0)
 S ^TMP("AFJX",$J,SSN,1,0)="@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@"
"RTN","AFJXPATS",13,0)
 S ^TMP("AFJX",$J,SSN,2,0)="SUMMARY NETWORK PATIENT LIST AS OF  "_$E(ASOF,4,5)_"-"_$E(ASOF,6,7)_"-"_$E(ASOF,2,3)_"@"_$E(ASOF,9,12)
"RTN","AFJXPATS",14,0)
 S ^TMP("AFJX",$J,SSN,3,0)="@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@"
"RTN","AFJXPATS",15,0)
 S ^TMP("AFJX",$J,SSN,4,0)=^XMB("NAME")_"  Patient File DPT(0) "_$G(^DPT(0)),^TMP("AFJX",$J,SSN,5,0)="    "
"RTN","AFJXPATS",16,0)
 Q
"RTN","AFJXPATS",17,0)
WRITE S CT=CT+1,REC=$G(^DPT(DFN,0)),NAM=$P(REC,U,1),SSN=$P(REC,U,9),DOB=$P(REC,U,3),^TMP("AFJX",$J,SSN,CT,0)=SSN_U_DOB_U_NAM_U_^XMB("NAME")_U_DFN
"RTN","AFJXPATS",18,0)
 Q
"RTN","AFJXPATS",19,0)
SEND ; send patient list
"RTN","AFJXPATS",20,0)
 D TOP,SITE
"RTN","AFJXPATS",21,0)
 K XMY S XMSUB="Summary Network Patients  From "_^XMB("NAME")
"RTN","AFJXPATS",22,0)
 S XMTEXT="^TMP(""AFJX"",$J,SSN,"
"RTN","AFJXPATS",23,0)
 D ^XMD
"RTN","AFJXPATS",24,0)
 K XMSUB,XMTEXT,XMY,^TMP("AFJX",$J,SSN)
"RTN","AFJXPATS",25,0)
 Q
"RTN","AFJXPATS",26,0)
SITE S DES="" F  S DES=$O(^AFJ(537025,"B",DES)),B="" Q:DES=""  F  S B=$O(^AFJ(537025,"B",DES,B)) Q:B=""  S INCL=$P($G(^AFJ(537025,B,0)),U,4) Q:INCL='1  S XMY("S.AFJXNETP@"_DES)="" W !,DES
"RTN","AFJXPATS",27,0)
 Q
"RTN","AFJXPATS",28,0)
 ;
...
"RTN","AFJXREW",2,0)
 ;;5.1;Network Health Exchange;**15,17,18,23**;Mar'99
"RTN","AFJXREW",3,0)
EN D INITIAL
"RTN","AFJXREW",4,0)
 I ('ERFLAG) D PROCESS
...
"RTN","AFJXTRF",2,0)
 ;;5.1; Network Health Exchange;**17,18,23**; Aug'99
"RTN","AFJXTRF",3,0)
